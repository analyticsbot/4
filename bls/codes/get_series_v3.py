...
for link in goodlinks:
    ...
    while True:
        ...
        try:
            print filename ,   3000000*i+skip, header
            df = pd.read_csv(filename, sep = '\t', nrows = 3000000, low_memory=False,  skiprows = 3000000*i+skip, header = header)
        except Exception,e:
...
##                    df1.columns = ['measure_code', 'Measure']
        ...
        try:
            df['footnote_codes'] = df['footnote_codes'] + df['footnote_code']
        except:
            pass
        try:
            df.drop('footnote_code', axis=1, inplace=True)
        except:
            pass
        ...
        for coll in col_names:
            if df[coll].isnull().sum() == nrows:
                df.drop(coll, axis=1, inplace=True)

            if (df[coll] == '').sum() == nrows:
                df.drop(coll, axis=1, inplace=True)
                                   
                
        try:
            df.to_csv(index = False, encoding = 'utf-8')
            print 'csv made', df.shape,filename, i, '###################'
        except  Exception, e:
