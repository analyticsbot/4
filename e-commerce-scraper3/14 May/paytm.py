...
class Paytm:
    ...
    def scrapeData(self, url, df):
        ...
        try:
            driver.find_element_by_css_selector('.ng-pristine.ng-valid.md-input.ng-valid-maxlength.ng-touched').send_keys('110001')
        ...
        try:
            for d in data:
                if 'Shipping Charges' in d.text:
                    Price_shipping = d.text.replace('Shipping Charges:','').replace('Rs','').strip()
                elif 'Free' in d.text:
                    Price_shipping = 0
        except:
            Price_shipping = ''
        ...
        try:
            ...
            Category_path = ''
        try:
            Description = driver.find_element_by_css_selector('.ProductDescription').text.replace('\n\n','\n').split('\n')
            dict_  = {}
            if Description[1] != 'Brand':
                for val in Description:
                    ix = Description.index(val)
                    if ix%2==1:                        
                        dict_[unidecode(val)] = unidecode(Description[ix+1])
            else:
