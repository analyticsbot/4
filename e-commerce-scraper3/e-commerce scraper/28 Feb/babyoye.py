...
class Babyoye:
    ...
    def scrapeData(self, url, df):
        ...
        try:
            ...
            Reviews = ''
        try:
            Status = driver.find_element_by_css_selector('.stockDetail.in-stock-details').find_element_by_css_selector('.head').text.strip().upper()
            if Status == '':
                Status = driver.find_element_by_css_selector('.temporarily_outof_stock.algncenter.fnt14s').find_element_by_css_selector('.fnt35.block').text.strip().upper()
        except:
            Status =""
        Condition = 'NEW'
