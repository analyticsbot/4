...
class Infobeam:
    ...
    def scrapeData(self, url, df):
        ...
        try:
            ...
            Price_shipping = ''
        try:
            Delivery = driver.find_element_by_css_selector('.shipping_duration').text.replace('Ships in','').replace('Days','').strip()
        except:
            Delivery = ''
        try:
        ...
        try:
            EMI = driver.find_element_by_css_selector('.emi-text').text.strip()
        except:
            EMI = ''
        try:
            breadcrums = driver.find_element_by_css_selector('.breadcrumb-sdp.no-padding-xs').find_elements_by_tag_name('a')[:-1]
            b = ''
