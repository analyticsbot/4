...
class Jabong:
    ...
    def scrapeData(self, url, df):
        ...
        try:
            product_details = soup.find(attrs = {'class':'prod-main-wrapper'.text).findAll('li')
        ...
        try:
            soup.find_element_by_id('pdp-zip-link').click()
            d = soup.find_element_by_id('pdp-tat')
            a = d.find('input')
            a.send_keys('110001')
            time.sleep(1)
        except:
            pass
        try:
            soup.find(attrs = {'class':'input-group-addon btn-primary change-pincode'}).click()
        except:
            pass
        Price_shipping = ''
        try:
            Delivery = soup.find_element_by_id('pdp-tat-tool-tip'}).getText()#.split('|')[0]
        except:
            Delivery = ''
        try:
            COD = soup.find_element_by_id('pdp-tat-tool-tip'}).getText()#.split('|')[1].strip()
        except:
            COD = ''
        EMI = ''
        try:
