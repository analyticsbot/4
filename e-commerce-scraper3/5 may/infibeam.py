...
class Infobeam:
    ...
    def scrapeData(self, url, df):        
        ...
        URL_raw = url
        try:
            Title = soup.find(attrs = {'id':'title'}).getText().replace('"','')
        except:
            Title = ''
        try:
            Brand = Title.split()[0]
        ...
        try:
            ...
            Seller = ''
        try:
            IMG_medium = soup.find(attrs = {'class':'img-responsive.inview'})['src']
        except:
            IMG_medium = ''
        try:
        ...
        try:
            zipcode = soup.find_element_by_id('zipCode')
            zipcode.send_keys('110001')
            soup.find_element_by_id('zipCheckSubmit').click()
        except:
            pass
        try:
        ...
        try:
            ...
            Delivery = ''
        try:
            COD = soup.find(attrs = {'class':'stockDetailRow.stockCodDEtail'}).getText().strip()
            ...
            try:
                COD  = soup.find(attrs = {'class':'codSpan'}).getText().strip()
                if 'is available' in COD:
                    COD = 'Available'
                else:
                    COD = 'Not Available'
