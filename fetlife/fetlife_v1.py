...
def getUserDetails(user_url, info, location, username, con, meta, user_exists):
    ...
    user_id = (con.execute(users.select().where(users.c.nickname == username))).fetchone()['id']
    ...
    try:
        ...
        for website in Websites:
            website_details  = {'website_name':group, 'user_id':user_id}
            if not user_exists:
                con.execute(meta.tables['websites'].insert(), website_details)
            elif user_exists:
                con.execute(meta.tables['websites'].update().where(users.c.id == user_id), website_details)
    except:
        pass
    ...
    try:
        ...
        for group in Groups_member_of:
            group_details  = {'group_name':group, 'user_id':user_id}
            if not user_exists:
                con.execute(meta.tables['groups'].insert(), group_details)
            elif user_exists:
                con.execute(meta.tables['groups'].update().where(users.c.id == user_id), group_details)
    except:
        pass
        ...
        try:
            ...
            for value in values:
                ...
                fetish_details = {'option':option, 'the_fetish':the_fetish,\
                                  'how_they_like_it':how_they_like_it, 'user_id':user_id}
            
                if not user_exists:
                    con.execute(meta.tables['fetishes'].insert(), fetish_details)
                elif user_exists:
                    con.execute(meta.tables['fetishes'].update().where(users.c.id == user_id), fetish_details)
        except:
            pass
    ...
    try:
        ...
        for i in s:
            ...
            if newonly == 1 and user_exists:
                ...
                getUserDetails(user_url, info, location, username, con, meta, user_exists)
