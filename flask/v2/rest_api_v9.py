...
##    df = psql.read_sql(sql, connection)
    ...
    for i in range(1, num_features):
        ...
        results = model.fit()
        print list(results.params      )[0]>0.0
        print list(results.params      )[0]
...
##        if all_positive and regr.intercept_< 0.0:
##            continue
##        
##        if  all_negative and regr.intercept_>= 0.0:
##            continue
##
##        if abs(max(pnl_values)) < abs(regr.intercept_):
##            continue
