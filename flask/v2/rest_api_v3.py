...
def equationsLR(ScenarioSetId, numEqn):
    ...
    dfs = []
    while True:
      sql = "SELECT [CURVE_ID],[CURVE_NM],[IDX_NM],[GRID_MTH_DT],[INPUT_PR_AMT],[PNL],[ScenarioSetId]FROM [PrimeOne].[dbo].[VW_MLScenarioSetProdCurveDetail]   where ScenarioSetId = %d" % (ScenarioSetId) 
      dfs.append(psql.read_frame(sql, connection))
      offset += chunk_size
