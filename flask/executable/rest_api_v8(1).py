...
def equationsLR(ScenarioSetId, numEqn, allmonth, maxvars, maxval):
    ...
    df['grid_col'] = df.apply (lambda row: row['IDX_NM'] + ' ' + str(dateutil.parser.parse(row['GRID_MTH_DT']).strftime('%Y_%m')),axis=1)
    ...
    for i in range(1, num_features):
        ...
        all_greater = all(i >= maxval for i in coef)
...
def getEquations():
    ...
    sample request = http://127.0.0.1:5000/getEquations?ScenarioSetId=7&numEqn=10&allmonth=1&maxvars=5&maxval=10000
    """
    ...
    maxval = int(request.args.get('maxval'))

    eqns = equationsLR(ScenarioSetId , numEqn, allmonth, maxvars, maxval)
